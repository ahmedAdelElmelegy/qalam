...
class TraditionStoryView extends StatefulWidget {
  final TraditionModel tradition;
  final String languageCode;
  final VoidCallback onClose;

  const TraditionStoryView({
    super.key,
  ...
  @override
  State<TraditionStoryView> createState() => _TraditionStoryViewState();
}

class _TraditionStoryViewState extends State<TraditionStoryView> {
  ...
  Widget build(BuildContext context) {
    final List<String> arabicContent = widget.tradition.getDetails('ar');
    final List<String> userLangContent = widget.tradition.getDetails(
      widget.languageCode,
    );

    final List<String> arParagraphs = arabicContent.isNotEmpty
        ? arabicContent
        : [widget.tradition.getDescription('ar')];

    final List<String> userLangParagraphs = userLangContent.isNotEmpty
        ? userLangContent
        : [widget.tradition.getDescription(widget.languageCode)];
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              ...
              background: Stack(
                fit: StackFit.expand,
                children: [
                  if (widget.tradition.gallery.isNotEmpty)
                    TraditionImage(
                          imagePath: widget.tradition.gallery.first,
                          fit: BoxFit.cover,
                        )
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (widget.languageCode != 'ar') ...[
                          SizedBox(height: 8.h),
                          Text(
                            widget.tradition.getTitle(widget.languageCode),
          ...
          if (widget.tradition.videoIdea.isNotEmpty)
            SliverToBoxAdapter(
              child: Padding(
                padding: EdgeInsets.all(24.w),
                child: widget.tradition.videoIdea.startsWith('http')
                    ? Column(
                        ...
                        children: [
                          ...
                          VideoThumbnailCard(
                            videoUrl: widget.tradition.videoIdea,
                            title: widget.tradition.getTitle(
                              widget.languageCode,
                            ),
                          ),
                        ],
